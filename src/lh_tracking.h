...
public:
    static TrackStrategyParam getDefaultFaceTrackStrategyParam(int picture_width, int picture_height) {
        ...
        param.kMaxFrameIntervalKeep = 30;
...
public:
    ...
    void Update(vector<DetectObject *> &detectobject_set, vector<unsigned long> &kill_id,int current_frame_index) {
        // get frame id
//        unsigned long current_frame_index = 0;
        auto trackobject_set = track_system_->getALiveTrackObjects();
        LOG(INFO)<<"trackobject_set size:"<<trackobject_set.size();
...
public:
    ...
    virtual void Update(const cv::Mat &img, const unsigned long &frm_id,
        ...
        if (is_key_frame) {
            ...
            trackStrategy->Update(detectobject_set, kill_id, frm_id);
